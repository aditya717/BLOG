...
function fetchDetails(
    ...
    sessionStorage.setItem('otherSkills', 0);
    ...
    blogs.forEach(element => {
        document.getElementById(`${element["_id"]}-delete`).addEventListener('click', async function () {
            try {
                ...
                const response = await fetch('/blog/delete/pe', params);
...
async function fetchBlog() {
    try {
        ...
        const response = await fetch(`/blog/getMyPost/${val}`, params);
        if (response.status == 200 || response.status == 201) {
            const blog = await response.json();
